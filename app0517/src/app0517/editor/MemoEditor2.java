...
public class MemoEditor2 extends JFrame{
	JMenuBar bar;
	JMenu m_file;
	...
	public void openFile() {
		...
		if(res==JFileChooser.APPROVE_OPTION) { 
			...
			BufferedReader buffer=null; //문자스트림을 대상으로 버퍼를 지원하는 스트림
			
			try {
				reader = new FileReader(file);//파일을 대상으로 빨대 꽂음
				buffer = new BufferedReader(reader); //버퍼처리된 스트림으로 빨대업그레이드
				
				String data=null; //읽혀진 문자(정확히는 문자의 코드)를 담게될 정수
				
				while(true) {
					data = buffer.readLine(); //한 문자열 즉 한 줄(케릭터들을 모아서) 읽기
					if(data==null)break;
					area.append(data+"\n");
				}
